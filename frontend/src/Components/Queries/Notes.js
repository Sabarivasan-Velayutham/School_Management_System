...
const Notes = () => {
  const { group, notes, setNotes, user } = useContext(UserContext);
  const [error, setError] = useState("");

  useEffect(() => {
    const getNotes = async () => {
      try {
        const response = await axios.get("notes/group/" + group._id);
    ...
    <main>
      ...
      <ul className="grid grid-cols-1 font-semibold sm:grid-cols-2 lg:flex lg:items-center lg:justify-start lg:gap-16">
        {user.userType === "student" && (
          <li className="p-1">Teacher : {group.teacher.name}</li>
        )}
        <li>
          <Link
              ...
              <summary className="list-none ">
                <div className="flex justify-between">
                  ...
                  <div className="flex p-3 pb-1">
                    <a href={note.attachment}>Download File</a>
                    {/* <a href={note.attachment}>{note.attachment}</a> */}

                    {/* <Link to={`${index}/edit`} id={index}>
                        <FaEdit className="r ounded-md ml-2 p-1 text-3xl hover:bg-violet-900 hover:text-slate-100 dark:hover:bg-violet-600 lg:p-2 lg:text-4xl" />
                      </Link> */}
                    {user.userType === "teacher" && (
                      <Link
                        id={note._id}
